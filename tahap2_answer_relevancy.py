...
def main():
    ...
    print(f"  Metrik      : Answer Relevancy")
    ...
    metrik_daftar = [
        ("Answer Relevancy", "GENERATION", AnswerRelevancyMetric(threshold=0.5, model=judge, include_reason=True, async_mode=False)),
    ]
    ...
    for nama, sisi, metrik in metrik_daftar:
        ...
        print(f"  [Info] Menggunakan Env Key: Lokal (Ollama)")
    ...
    out = f"hasil_evaluasi_AnswerRelevancyMetric_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
