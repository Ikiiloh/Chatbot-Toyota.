...
def main():
    ...
    print(f"  Metrik      : Contextual Recall")
    ...
    metrik_daftar = [
        ("Contextual Recall", "RETRIEVAL", ContextualRecallMetric(threshold=0.5, model=judge, include_reason=True, async_mode=False)),
    ]
    ...
    out = f"hasil_evaluasi_ContextualRecallMetric_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
