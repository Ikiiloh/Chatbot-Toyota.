...
# =====================================================================
load_dotenv()
MODEL_JUDGE = "google/gemma-4-31b-it:free"
API_KEY_JUDGE = os.getenv("OPENROUTER_API_KEY")
INPUT_FILE = "hasil_rag_pipeline.json"
JEDA = 5  # 5 detik antar evaluasi (OpenRouter lebih cepat)
...
def main():
    ...
    print(f"  Metrik      : Contextual Precision")
    print(f"  LLM Judge   : OpenRouter ({MODEL_JUDGE})")
    print(f"  API Key Env : OpenRouter")
    print("=" * 60)
    ...
    judge = OpenRouterJudge(model_name=MODEL_JUDGE, api_key=API_KEY_JUDGE)
    judge.set_api_key(API_KEY_JUDGE)
    ...
    metrik_daftar = [
        ("Contextual Precision", "RETRIEVAL", ContextualPrecisionMetric(threshold=0.5, model=judge, include_reason=True, async_mode=False)),
    ]
    ...
    for nama, sisi, metrik in metrik_daftar:
        print(f"\n[{sisi}] {nama}")
        key_masked = API_KEY_JUDGE[:15] + "..." if API_KEY_JUDGE else "N/A"
        print(f"  [Info] Menggunakan Env Key: {key_masked}")
    ...
    laporan = {
        ...
        "model_judge": f"OpenRouter ({MODEL_JUDGE})",
    ...
    out = f"hasil_evaluasi_ContextualPrecisionMetric_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
