...
def main():
    ...
    metrik_daftar = [
        ("Faithfulness", "GENERATION", FaithfulnessMetric(threshold=0.5, model=judge, include_reason=True, async_mode=False)),
    ]
    ...
    out = f"hasil_evaluasi_FaithfulnessMetric_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
